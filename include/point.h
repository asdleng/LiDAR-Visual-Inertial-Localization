...
public:
  ...
  list<FeaturePtr>              obs_;                     //被很多个关键帧看到，这个list存放这些关键帧
  ...
  bool getFurthestViewObs(const Vector3d& pos, FeaturePtr& obs, const Vector2d& cur_px) const;
  bool getCloseViewObs(const Vector3d& pos, FeaturePtr& obs, const Vector2d& cur_px) const;
  bool getCloseViewObsBesidesSelf(Frame* frame, std::vector<FeaturePtr>& ftr_list, const Vector2d& cur_px, int max_ftr_list_num) const;
