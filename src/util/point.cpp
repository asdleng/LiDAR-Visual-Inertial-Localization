...
bool Point::getCloseViewObsBesidesSelf(Frame* frame, std::vector<FeaturePtr>& ftr_list, const Vector2d& cur_px, int max_ftr_list_num) const
{
  // TODO: get frame with same point of view AND same pyramid level!
  //std::cout<<"观测次数："<<obs_.size()<<std::endl;
  if(obs_.size() ==0) return false;
  ftr_list.clear();
  //std::cout<<"2"<<std::endl;
  Vector3d obs_dir(frame->pos() - pos_); obs_dir.normalize();
  auto min_it=obs_.begin();
  double min_cos_angle = 0;
 //std::cout<<"3"<<std::endl;
  for(auto it=obs_.begin(), ite=obs_.end(); it!=ite; ++it)
  {
    if(it->get()->frame->id_==frame->id_) continue; // 跳过当前帧
    Vector3d dir((*it)->pos() - pos_); dir.normalize();  // 注意这里是这个特征点所在关键帧的位姿
    double cos_angle = obs_dir.dot(dir);  // 作内积
    //std::cout<<cos_angle<<std::endl;
  if(cos_angle > 0.5) // assume that observations larger than 60° are useless 0.5
  {
      ftr_list.push_back(*it);
    }
    if(ftr_list.size()>=max_ftr_list_num) break; // 不多于5个
  }
  ...
  //   return false;    
  // }
  //std::cout<<"最小共视角度为："<<acos(min_cos_angle)*180/PI_M<<std::endl;
  if(ftr_list.size() ==0 ) // assume that observations larger than 60° are useless 0.5
  {
    // ROS_ERROR("The obseved angle is larger than 60°.");
    return false;
  }
...
bool Point::getCloseViewObs(const Vector3d& framepos, FeaturePtr& ftr, const Vector2d& cur_px) const
{
  ...
  for(auto it=obs_.begin(), ite=obs_.end(); it!=ite; ++it)
  {
    Vector3d dir((*it)->T_f_w_.inverse().translation() - pos_); dir.normalize();  // 注意这里是这个特征点所在关键帧的位姿
    double cos_angle = obs_dir.dot(dir);  // 作内积
  ...
  //   return false;    
  // }
  //std::cout<<"最小共视角度为："<<min_cos_angle<<std::endl;
...
bool Point::getFurthestViewObs(const Vector3d& framepos, FeaturePtr& ftr, const Vector2d& cur_px) const
{
  // TODO: get frame with same point of view AND same pyramid level!
  if(obs_.size() <= 0) return false;
  ...
  for(auto it=obs_.begin(), ite=obs_.end(); it!=ite; ++it)
  {
    Vector3d dir((*it)->T_f_w_.inverse().translation() - pos_); dir.normalize();  // 注意这里是这个特征点所在关键帧的位姿
    double cos_angle = obs_dir.dot(dir);  // 作内积
    if(cos_angle < max_cos_angle&&cos_angle > 0.5)
    {
  ...
  //   return false;    
  // }
  //std::cout<<"最小共视角度为："<<max_cos_angle<<std::endl;
